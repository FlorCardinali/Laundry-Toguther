...
public class MaquinaManager : NetworkBehaviour
{
    ...
    public GameObject prefabRopaDevuelta;
    ...
    private IEnumerator RutinaTrabajo()
    {
        ...
        if (esLavarropas && modoLavado.Value == reqLavado.Value) corrLavado.Value = true;
        if (!esLavarropas && modoLavado.Value == reqSecado.Value) corrSecado.Value = true;
    ...
    {
        GameObject nuevaRopa = Instantiate(prefabRopaDevuelta, transform.position, Quaternion.identity);
        nuevaRopa.GetComponent<NetworkObject>().Spawn();

        ItemRopa scriptRopa = nuevaRopa.GetComponent<ItemRopa>();
        scriptRopa.lavadoRequerido.Value = reqLavado.Value;
        scriptRopa.secadoRequerido.Value = reqSecado.Value;
        scriptRopa.lavadoCorrectamente.Value = corrLavado.Value;
        scriptRopa.secadoCorrectamente.Value = corrSecado.Value;
